...
q_rf <- q_mean_logs %>%
  select(q_mean10, p_log10, aridlog10, soil_porosity,
         soil_depth_pelletier, max_water_content, organic_frac, frac_snow, pet_mean) %>%
  na.omit()
...
test$cont$rf_pred <- predict(rf_numer, test_cont)

ggplot(test_cont, aes(x = q_mean10, y = rf_pred))+
  geom_point()+
  geom_abline(slope = 1)
