...
q_mean_logs <- chydro %>%
  mutate(q_log10=log10(q_mean)) %>%
  mutate(runofflog10 = log10(runoff_ratio))
...
#create dataset
c_data <- q_mean_logs %>%
   select(q_log10, p_mean, runofflog10, frac_snow, low_prec_freq, low_q_dur ) %>%
  #to make it a classification tree - converting data from continuous to a class (binning it) -> gives us equal bins in each classification, which is necessary for machine learning
...
cart_simple_1 <- rpart(q_class ~ ., data = train, cp = 0.001, method = 'class')

#visualize table/classes  
table(c_data$q_class)
